...
namespace com.fasterxml.jackson.core.json
{
	...
	/// that outputs JSON content using a
	/// <see cref="System.IO.TextWriter"/>
	/// which handles character encoding.
	/// </summary>
	public sealed class WriterBasedJsonGenerator : com.fasterxml.jackson.core.json.JsonGeneratorImpl
	{
		protected internal const int SHORT_WRITE = 32;

		protected internal static readonly char[] HEX_CHARS = com.fasterxml.jackson.core.io.CharTypes
			.copyHexChars();

		protected internal readonly System.IO.TextWriter _writer;
		...
		/// .
		/// </summary>
		protected internal char[] _outputBuffer;

		/// <summary>Pointer to the first buffered character to output</summary>
		protected internal int _outputHead = 0;
		...
		*/
		...
		/// <exception cref="System.IO.IOException"/>
		protected internal void _writeFieldName(string name, bool commaBefore)
		{
			if (_cfgPrettyPrinter != null)
			{
		...
		/// <exception cref="System.IO.IOException"/>
		/// <exception cref="com.fasterxml.jackson.core.JsonGenerationException"/>
		protected internal void _writePPFieldName(string name, bool commaBefore)
		{
			if (commaBefore)
			{
		...
		*/
		...
		public override void writeRaw(string text)
		{
			...
			if (room >= len)
			{
				Sharpen.Runtime.getCharsForString(text, 0, len, _outputBuffer, _outputTail);
				_outputTail += len;
			}
			else
			{
		...
		/// <exception cref="System.IO.IOException"/>
		public override void writeRaw(string text, int start, int len)
		{
			// Nothing to check, can just output as is
			int room = _outputEnd - _outputTail;
			if (room < len)
			...
			if (room >= len)
			{
				Sharpen.Runtime.getCharsForString(text, start, start + len, _outputBuffer, _outputTail
					);
				_outputTail += len;
			}
			else
			{
				writeRawLong(Sharpen.Runtime.substring(text, start, start + len));
			}
		}
		...
		public override void writeRaw(char[] text, int offset, int len)
		{
			...
			if (len < SHORT_WRITE)
			{
				...
				System.Array.Copy(text, offset, _outputBuffer, _outputTail, len);
				_outputTail += len;
				return;
			}
			// Otherwise, better just pass through:
		...
		private void writeRawLong(string text)
		{
			...
			Sharpen.Runtime.getCharsForString(text, 0, room, _outputBuffer, _outputTail);
			_outputTail += room;
			_flushBuffer();
			int offset = room;
			...
			while (len > _outputEnd)
			{
				...
				Sharpen.Runtime.getCharsForString(text, offset, offset + amount, _outputBuffer, 0
					);
				_outputHead = 0;
				_outputTail = amount;
				_flushBuffer();
			...
			Sharpen.Runtime.getCharsForString(text, offset, offset + len, _outputBuffer, 0);
			_outputHead = 0;
			_outputTail = len;
		}
		...
		*/
		...
		{
			if (_cfgNumbersAsStrings || (isEnabled(com.fasterxml.jackson.core.JsonGenerator.Feature
				.QUOTE_NON_NUMERIC_NUMBERS) && ((double.IsNaN(d) || double.isInfinite(d)))))
			{
				// [JACKSON-139]
		...
		{
			if (_cfgNumbersAsStrings || (isEnabled(com.fasterxml.jackson.core.JsonGenerator.Feature
				.QUOTE_NON_NUMERIC_NUMBERS) && ((float.IsNaN(f) || float.isInfinite(f)))))
			{
				// [JACKSON-139]
		...
		*/
		/// <exception cref="System.IO.IOException"/>
		protected internal override void _verifyValueWrite(string typeMsg)
		{
			if (_cfgPrettyPrinter != null)
			{
		...
		/// <exception cref="System.IO.IOException"/>
		protected internal void _verifyPrettyValueWrite(string typeMsg)
		{
			int status = _writeContext.writeValue();
			if (status == com.fasterxml.jackson.core.json.JsonWriteContext.STATUS_EXPECT_NAME)
			{
...
escape_loop_break: ;
				...
				* we have room now.
				*/
				char c_1 = _outputBuffer[_outputTail++];
				_prependOrWriteCharacterEscape(c_1, escCodes[c_1]);
...
output_loop_break: ;
		...
		private void _writeLongString(string text)
		{
			...
			do
			{
				...
				Sharpen.Runtime.getCharsForString(text, offset, offset + segmentLen, _outputBuffer
					, 0);
				if (_characterEscapes != null)
				{
		...
		/// to the output buffer prior to call.
		/// </summary>
		/// <remarks>
		/// Method called to output textual context which has been copied
		...
		private void _writeString(char[] text, int offset, int len)
		{
			...
			*/
			...
			while (offset < len)
			{
				...
				char c_1 = text[offset++];
				_appendCharacterEscape(c_1, escCodes[c_1]);
			}
		}
