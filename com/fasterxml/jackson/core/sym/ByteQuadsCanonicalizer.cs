...
namespace com.fasterxml.jackson.core.sym
{
	...
	/// </remarks>
	/// <since>2.6</since>
	public sealed class ByteQuadsCanonicalizer
	{
		/// <summary>Initial size of the primary hash area.</summary>
		/// <remarks>
		...
		/// </remarks>
		protected internal readonly java.util.concurrent.atomic.AtomicReference<com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer.TableInfo
			> _tableInfo;
		...
		/// of collisions).
		/// </summary>
		protected internal int[] _hashArea;
		...
		/// </summary>
		protected internal int _hashSize;
		...
		/// </summary>
		protected internal int _secondaryStart;
		...
		/// </summary>
		protected internal int _tertiaryStart;
		...
		/// bigger table sizes.
		/// </remarks>
		protected internal int _tertiaryShift;

		/// <summary>Total number of Strings in the symbol table; only used for child tables.
		/// 	</summary>
		...
		/// Pointer to the offset within spill-over area where there is room
		/// for more spilled over entries (if any).
		/// Spill over area is within fixed-size portion of
		/// <see cref="_hashArea"/>
		/// .
		/// </remarks>
		protected internal int _spilloverEnd;
		...
		/// ).
		/// </summary>
		protected internal int _longNameOffset;
		...
		*/
		...
		/// </summary>
		protected internal static com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer createRoot
			(int seed)
		{
			return new com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer(DEFAULT_T_SIZE, 
				true, seed, true);
		}
		...
		/// use for parsing.
		/// </summary>
		public com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer makeChild(int flags)
		{
			return new com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer(this, com.fasterxml.jackson.core.JsonFactory.Feature
		...
		public void release()
		{
			...
			{
				_parent.mergeChild(new com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer.TableInfo
					(this));
			 ...
			 childState)
		{
			...
			com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer.TableInfo currState = _tableInfo
				.get();
			...
			if (childCount > MAX_ENTRIES_FOR_REUSE)
			{
				...
				childState = com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer.TableInfo.createInitial
					(DEFAULT_T_SIZE);
			}
		...
		*/
		...
		/// <summary>Returns number of primary slots table has currently</summary>
		...
		public override string ToString()
		{
			...
			return Sharpen.Extensions.StringFormat("[%s: size=%d, hashSize=%d, %d/%d/%d/%d pri/sec/ter/spill (=%s), total:%d]"
				, GetType().FullName, _count, _hashSize, pri, sec, tert, spill, total, (pri + sec
				 + tert + spill), total);
		}
		...
		*/
		...
		public string findName(int q1, int q2, int q3)
		{
			...
			if (len == 3)
			{
				if ((q1 == hashArea[offset]) && (hashArea[offset + 1] == q2) && (hashArea[offset 
					+ 2] == q3))
				{
					return _names[offset >> 2];
				}
			...
			if (len == 3)
			{
				if ((q1 == hashArea[offset2]) && (hashArea[offset2 + 1] == q2) && (hashArea[offset2
					 + 2] == q3))
				{
					return _names[offset2 >> 2];
				}
		...
		*/
		...
		private string _findSecondary(int origOffset, int q1, int q2)
		{
			...
			for (offset = _spilloverStart(); offset < _spilloverEnd; offset += 4)
			{
				if ((q1 == hashArea[offset]) && (q2 == hashArea[offset + 1]) && (2 == hashArea[offset
					 + 3]))
				{
					return _names[offset >> 2];
				}
		...
		private string _findSecondary(int origOffset, int q1, int q2, int q3)
		{
			...
			{
				int len = hashArea[offset + 3];
				if ((q1 == hashArea[offset]) && (q2 == hashArea[offset + 1]) && (q3 == hashArea[offset
					 + 2]) && (3 == len))
				{
					return _names[offset >> 2];
				}
		...
		private bool _verifyLongName(int[] q, int qlen, int spillOffset)
		{
			int[] hashArea = _hashArea;
		...
		private bool _verifyLongName2(int[] q, int qlen, int spillOffset)
		{
			int ix = 0;
			do
			{
		...
		*/
		...
		private void _verifySharing()
		{
			if (_hashShared)
			{
				_hashArea = java.util.Arrays.copyOf(_hashArea, _hashArea.Length);
				_names = java.util.Arrays.copyOf(_names, _names.Length);
				_hashShared = false;
			}
			if (_needRehash)
			{
		...
		/// <summary>Method called to find the location within hash table to add a new symbol in.
		/// 	</summary>
		...
		private int _appendLongName(int[] quads, int qlen)
		{
			...
			if ((start + qlen) > _hashArea.Length)
			{
				...
				int minAdd = System.Math.min(4096, _hashSize);
				int newSize = _hashArea.Length + System.Math.max(toAdd, minAdd);
				_hashArea = java.util.Arrays.copyOf(_hashArea, newSize);
			}
			System.Array.Copy(quads, 0, _hashArea, start, qlen);
		...
		*/
		private void rehash()
		{
			...
			*/
			...
			for (int offset = 0; offset < end; offset += 4)
			{
				...
				switch (len)
				{
					...
					default:
					{
						...
						System.Array.Copy(oldHashArea, qoff, q, 0, len);
						addName(name, q, len);
						break;
					}
				}
			...
			{
				throw new System.InvalidOperationException("Failed rehash(): old count=" + oldCount
					 + ", copyCount=" + copyCount);
			}
		}
		...
		/// arrays allocated
		/// </summary>
		private void nukeSymbols(bool fill)
		{
			_count = 0;
			...
			if (fill)
			{
				java.util.Arrays.fill(_hashArea, 0);
				java.util.Arrays.fill(_names, null);
			}
		}
		...
		/**********************************************************
		*/
		/// <summary>Helper method that calculates start of the spillover area</summary>
		...
		protected internal void _reportTooManyCollisions()
		{
			...
			throw new System.InvalidOperationException("Spill-over slots in symbol table with "
				 + _count + " entries, hash area of " + _hashSize + " slots is now full (all " +
		...
		internal static int _calcTertiaryShift(int primarySlots)
		{
		...
		private sealed class TableInfo
		{
			...
			public readonly int tertiaryShift;

			public readonly int[] mainHash;

			public readonly string[] names;

			public readonly int spilloverEnd;

			public readonly int longNameOffset;

			public TableInfo(int size, int count, int tertiaryShift, int[] mainHash, string[]
				 names, int spilloverEnd, int longNameOffset)
			{
				/*
				/**********************************************************
				/* Helper classes
			...
			public TableInfo(com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer src)
			{
				size = src._hashSize;
				count = src._count;
			...
			public static com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer.TableInfo createInitial
				(int sz)
			{
				int hashAreaSize = sz << 3;
				int tertShift = _calcTertiaryShift(sz);
				return new com.fasterxml.jackson.core.sym.ByteQuadsCanonicalizer.TableInfo(sz, 0, 
					tertShift, new int[hashAreaSize], new string[sz << 1], hashAreaSize - sz, hashAreaSize
					);
			}
			// hashSize
			// count
			// mainHash, 2x slots, 4 ints per slot
			// 2x slots
			// at 7/8 of the total area
			// longNameOffset, immediately after main hashes
		}
	}
}
